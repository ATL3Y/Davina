...
#if UNITY_SINGLE_PASS_STEREO
		float2 screenParamsAdjusted = _CameraDepthTexture_TexelSize.zw;
#else
		float2 screenParamsAdjusted = _ScreenParams.xy;
#endif
		float index = frac(tex2Dlod(Tile5x5, float4(fmod((screenParamsAdjusted * VolumeScale) * uv, HxTileSize) / HxTileSize, 0, 0)).r + hxRayOffset);
