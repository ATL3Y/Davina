Shader "Custom/RainShader"
{
    Properties
	{
		...
		_AlphaRange("Alpha Range" , Vector) = (0.3,1,0,0)
		_DistancePower("Distance Power" , float) = 1.0
    }

    SubShader
	{
        Tags { "QUEUE"="Transparent+2000" "IGNOREPROJECTOR"="true" "RenderType"="Transparent" "LightMode"="Vertex" }
		LOD 100
...
//				float4 lightColor : TEXCOORD2;
			...
			float3 ApplyLight(int index, float3 lightColor, float3 viewPos)
			{
				...
				else 
				{
	                ...
	                fixed atten = 1.0 / (1.0 + ( pow( lengthSq , _DistancePower) * unity_LightAtten[index].z) );
            ...
            fixed4 LightForVertex(float4 vertex)
            {
                ...
                fixed3 lightColor = ( UNITY_LIGHTMODEL_AMBIENT.rgb + 0.1 ) * _AmbientLightMultiplier;
...
//				lightColor = ApplyLight(3, lightColor, viewPos);
                ...
                return fixed4(lightColor, lightColor.r );
            }
...
//                o.color = LightForVertex(v.vertex);
//                return o;
				...
				o.color.rgba = lerp(_AlphaRange.xxxx , _AlphaRange.yyyy , o.color.rgba);
