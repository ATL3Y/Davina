        ...
        if (t.GetComponent<MeshFilter>() != null)
        {
           ...
           // if (debug) AxKDebugLines.AddOBB(obb, Color.white);
            ...
            for (int i = 0; i < tris.Length; i++)
            {
                ...
                if (debug)
                {
                    AxKDebugLines.AddLine( points[ ( int )tris[ i ].x ], points[ ( int )tris[ i ].y ], d ? selectColor : defaultColor, 0 );
                    AxKDebugLines.AddLine( points[ ( int )tris[ i ].y ], points[ ( int )tris[ i ].z ], d ? selectColor : defaultColor, 0 );
                    AxKDebugLines.AddLine( points[ ( int )tris[ i ].x ], points[ ( int )tris[ i ].z ], d ? selectColor : defaultColor, 0 );
                }
            }
        }
    }
