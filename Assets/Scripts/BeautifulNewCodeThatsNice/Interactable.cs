    ...
    void Check(ref bool flag, Transform t, Vector3 inputPosition, Ray inputLookDirection)
    {
        ...
        if (t.GetComponent<MeshFilter>() != null)
        {
            ...
            for (int i = 0; i < tris.Length; i++)
            {
                ...
                if (debug)
                {
                    AxKDebugLines.AddLine( points[ ( int )tris[ i ].x ], points[ ( int )tris[ i ].y ], d ? selectColor : Color.gray, 0 );
                    AxKDebugLines.AddLine( points[ ( int )tris[ i ].y ], points[ ( int )tris[ i ].z ], d ? selectColor : Color.gray, 0 );
                    AxKDebugLines.AddLine( points[ ( int )tris[ i ].x ], points[ ( int )tris[ i ].z ], d ? selectColor : Color.gray, 0 );
                }
            }
        }
    }
